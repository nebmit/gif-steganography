...
def main():
    ...
    encode_parser.add_argument("input_file", type=str, help="Path to the input GIF file.")
    encode_parser.add_argument("output_file", type=str, help="Path to the output GIF file.")
    encode_parser.add_argument("text", type=str, help="Text data to be encoded into the GIF.")
    encode_parser.add_argument("passphrase", type=str, help="Passphrase to be used for encoding.")
    encode_parser.add_argument("--nsym", type=int, default=10, help="Factor for error correction (default: 10)")


    # Add arguments for decode command
    decode_parser = subparsers.add_parser('decode', help='Decode data from a GIF')
    decode_parser.add_argument("input_file", type=str, help="Path to the input GIF file.")
    decode_parser.add_argument("passphrase", type=str, help="Passphrase to be used for decoding.")
    decode_parser.add_argument("--nsym", type=int, default=10, help="Factor for error correction (default: 10)")
    ...
    if args.command == 'encode':
        ...
        try:
            data, is_corrupt = decode_encrypted(args.input_file, args.passphrase, args.nsym)
            if is_corrupt:
